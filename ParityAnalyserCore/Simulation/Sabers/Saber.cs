...
namespace ParityAnalyserCore.Sim
{
    public abstract class Saber
    {
        ...
        protected virtual IEnumerable<SaberSnapshot> ExploreBombGroup(BombGroup group, float currentTime, float endTime)
        {
            ...
            foreach ((BombCluster cluster1, BombCluster cluster2) in group.GetClusterPairs(ParityAnalyser.options.bombClusterMerging))
            {
                ...
                foreach (OrientedRect hitbox in cluster1.GetHitbox())
                {
                    ...
                    { 
                        bool swingPathCollides = Collision.SwingPathIntersects(hilt.ToVector2(), Math.Lerp(startAngle, endAngle, cluster1LerpAmount), Math.Lerp(startAngle, endAngle, cluster2LerpAmount), hitbox, debug, cluster1.aBomb);
		...
		public void RenderBombGroups(bool renderStart = false, bool renderEnd = false)
		{
			...
			{
				Vector3 groupColor = Utils.RandomColor();
				if (renderStart) DebugRenderer.RenderSphere(new Vector3(group.startNote.PosX, 3, 0f), 0.5f, groupColor, group.startNote);
				foreach (BaseNote bomb in group.bombs)
				{
					DebugRenderer.AddOutline(bomb, groupColor);
				}
				if (renderEnd && group.endNote != null) DebugRenderer.RenderSphere(new Vector3(group.endNote.PosX, 4, 0f), 0.3f, groupColor, group.endNote);
			}
		}
        ...
        public bool RollsComfortably(float roll) => (wristAngle != 0f && preferredRollDirection == Math.Sign(roll))
                                                || (wristAngle == 0f && roll == 0f) || Math.Sign(roll) != Math.Sign(wristAngle);
