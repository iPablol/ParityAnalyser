...
using BombCondition = System.Func<ParityAnalyser.Sim.BaseNote, bool>;
...
namespace ParityAnalyser.Sim
{
    public abstract class Saber
    {
        ...
        public BaseNote PopStack()
        {
            ...
            hasReset = snap.reset;
            Console.WriteLine("\nStack popped:");
            Console.WriteLine($"Pos: {transform.position}, wa: {wristAngle}, parity: {parity.ToString()}, time: {snap.note?.JsonTime ?? 0f}\n");
            return snap.note;
        }
        ...
        private void DodgeBombs(BombGroup group, BombCluster currentCluster, bool debug = false)
        {
            ...
            float timeDistanceToNote = group.endNote.JsonTime - currentCluster.time,
                ...
                noteForce = (float)Math.Clamp(-Math.Log(timeDistanceToNote) * (distanceToNote / distanceDamp), 0, distanceToNote);
